...
function index({images} : {images:string[]}) {
  ...
  return (
    <View style = {{alignItems : 'center',width : '100%',backgroundColor : 'white',height : height*0.18,paddingTop : 3}}>
    <FlatList
    data={images}
    ...
    renderItem={(item)=>(
        ...
        style= {{width :width*0.5,height:height*0.20,resizeMode:'stretch'}}
    ...
    <View>
      <View style = {{flexDirection : 'row',alignItems : 'center', width: 30,height:12,justifyContent:'space-around'}}>
        {images.map((image,index) =>(
          <View key={index} style ={[styles.dot,{backgroundColor : activeIndex == index ? '#3d0c45': '#F2F0FD'}]} />
        ))}
      </View>
    </View>
    </View>
